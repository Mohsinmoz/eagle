...
//    $.ajax({
//        type: "Get",
//        url: "/RegisterUser?title=" + field.salutation + "&firstName=" + field.firstName + "&lastName=" + field.lastName + "&country=" + field.country + "&email=" + field.email + "&mobile="+(field.mobile).split(' ')[0].replace(/\+/g, '') + (field.mobile).split(' ')[1].replace(/^0+/, '') + "&nationality=" + field.nationality + "&interestedIn=" + field.intrestedin + "&ageGroup=" + field.agegroup + "&unitType=" + field.unittype + "&preferedCommunicationChannel=" + field.PreferedCommunicationChannel + "&investmentType=" + field.InvestmentType + "&offerings=" + field.NoOfBedroom,
...
function PostForm() {
    ...
    $.ajax({
        type: "GET",
        url: "/umbraco/surface/FrontEnd/RegisterUser?title=" + field.salutation + "&firstName=" + field.firstName + "&lastName=" + field.lastName + "&country=" + field.country + "&email=" + field.email + "&mobile=" + (field.mobile).split(' ')[0].replace(/\+/g, '') + (field.mobile).split(' ')[1].replace(/^0+/, '') + "&nationality=" + field.nationality + "&interestedIn=" + field.intrestedin + "&ageGroup=" + field.agegroup + "&unitType=" + field.unittype + "&preferedCommunicationChannel=" + field.PreferedCommunicationChannel + "&investmentType=" + field.InvestmentType + "&offerings=" + field.NoOfBedroom,
        // contentType: "application/json; charset=utf-8",
        // dataType: "json",
        success: function (data) {
            if (data == "true") {
...
function POSTMethodCall() {
    ...
    var LeadDetails = { "newLead": { "Salutation": field.salutation, "FirstName": field.firstName, "LastName": field.lastName, "CountryOfResidence":field.country, "CountryCode": "+"+ field.countrycode, "Email": field.email, "MobilePrefix": (field.mobile).split(' ')[0], "MobileNumber": (field.mobile).split(' ')[1].replace(/^0+/, ''), "Nationality": field.nationality, "NationalityCode": "+" + field.nationalitycode, "InterestedIn": field.intrestedin, "AgeGroup": field.agegroup, "UnitType": field.unittype, "PreferedCommunicationChannel": field.PreferedCommunicationChannel, "InvestmentType": field.InvestmentType, "NoOfBedrooms": field.NoOfBedroom, "Website": "www.eaglehills.com" } };
...
function ValidateForm() {
    ...
    isvalid = ValidateMobilewithaSpace(lblmobile, mobile) && isvalid;
...
function EmptyInputFields() {
    ...
    lastname.val('');
    // mobile.val('');
    mobile.val(mobile.val().substring(0, mobile.val().split(' ')[0].length + 1));
    email.val('');
    ...
    $("#chkbw_Studio").removeAttr('checked');
    $("#chkbw_bedroom_1").removeAttr('checked');
    $("#chkbw_Bedroom_2").removeAttr('checked');
    $("#chkbw_Bedroom_3").removeAttr('checked');
    $("#chkbw_Bedroom_4Plus").removeAttr('checked');
    $("#chkbw_Bedroom_5Plus").removeAttr('checked');
